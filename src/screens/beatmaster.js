...
import BeatMasterSynthesis from '../components/beatMasterSynthesis.js' ;
import BeatMasterSession from '../components/beatMasterSession.js' ;

import { db } from '../functions/firebase.js' ;

import '../styles/beatmaster.css' ;

const BeatMaster = props => {
  ...
  useEffect( () => {
    ...
    .then ( queryDocuments => {
        queryDocuments.docs.map( doc => {
            if ( doc.data().game.includes("BeatMaster") && doc.data().events !== undefined )
  ...
  return (
    <div className = 'beatMasterPageContainer'>

      <NavHeader history = {props.history} />
  
      <div className = "BeatMasterContainers">
